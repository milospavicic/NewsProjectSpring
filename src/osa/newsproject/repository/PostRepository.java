...
//	@Modifying
//	@Query(value="SELECT p.* FROM posts AS p INNER JOIN users AS u ON p.user_id=u.user_id LEFT OUTER JOIN comments AS c ON p.post_id = c.post_id LEFT OUTER JOIN post_tags AS t ON p.post_id=t.post_id LEFT OUTER JOIN tags AS ta ON ta.tag_id = t.tag_id WHERE ta.name like ?1 OR u.name like ?1  GROUP BY p.post_id ORDER BY COUNT(DISTINCT c.comment_id) DESC",nativeQuery=true)
//	List<Post> findAllByCommentsCountAndSearch(String param);
}
